...
initialize ({rootState},{ _agent,_deposits,_token }){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.initialize(_agent,_deposits,_token).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.initialize(_agent,_deposits,_token).send({
                                from: rootState.app.account,
...
repayment ({rootState},{ id,value }){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.repayment(id,value).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.repayment(id,value).send({
                                from: rootState.app.account,
                                gas: parseInt(gas * 1.2)
                            }).then(res => {
