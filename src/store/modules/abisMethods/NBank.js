		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('NBank', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
const actions = { 
allowToken ({rootState},{ param0 }){
					judgeToken(rootState)
					return new Promise((resolve,reject) => {
						state.token.methods.allowToken(param0).call().then(res => {
						resolve(res)
					}).catch(err=>{
